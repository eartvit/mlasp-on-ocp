...
for (( i=0; i<$RUNS; ++i)); do
    echo -e 'Running cycle '$(($i+1))' out of '$RUNS
    ...
    `tkn pipeline start auto-multipod-lt-wiremock --param connectionsLowerBound=10 --param connectionsUpperBound=30 --param durationLowerBound=60 --param durationUpperBound=600 --param scaleLowerBound=1 --param scaleUpperBound=15 --param timeout=5s --param outFormat=json --param URL=http://wiremock-mlasp.demo1.svc.cluster.local:8080/another_mock --param asyncResp=true --param asyncRespThreadsLowerBound=10 --param asyncRespThreadsUpperBound=25 >> tkn_lt_pipeline_$NOW.log 2>&1`
